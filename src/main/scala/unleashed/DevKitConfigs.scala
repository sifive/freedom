...
import sifive.fpgashells.devices.xilinx.xilinxvc707mig
import sifive.fpgashells.devices.xilinx.xilinxvcu118mig

import sifive.fpgashells.devices.xilinx.xilinxvc707mig.XilinxVC707MIGParams
import sifive.fpgashells.devices.xilinx.xilinxvcu118mig.XilinxVCU118MIGParams
...
class U500VC707DevKitConfig extends Config(
  ...
  new FreedomUVC707Config().alter((site,here,up) => {
    ...
    case xilinxvc707mig.MemoryXilinxDDRKey => XilinxVC707MIGParams(address = Seq(AddressSet(0x80000000L,0x40000000L-1))) //1GB
...
class U500VCU118DevKitConfig extends Config(
  ...
  new FreedomUVCU118Config().alter((site,here,up) => {
    case ErrorParams => ErrorParams(Seq(AddressSet(0x3000, 0xfff)), maxAtomic=site(XLen)/8, maxTransfer=128)
    case PeripheryBusKey => up(PeripheryBusKey, site).copy(frequency = 50000000) // 50 MHz hperiphery
    case xilinxvcu118mig.MemoryXilinxDDRKey => XilinxVCU118MIGParams(address = Seq(AddressSet(0x80000000L,0x80000000L-1))) //2GB
    case DTSTimebase => BigInt(1000000)
